...
float distFunc(vec3 p){
    float sp1 = sd_sphere(op_trans(p),3.0*sin(u_time));

    float box1= sd_box(op_trans(p),vec3(2.0));
...
void main() {
    ...
    const vec3 lightDir = vec3(0.1, 0.2, 0.3);
    vec2 p = (gl_FragCoord.xy * 2.0 - u_resolution.xy) / min(u_resolution.x, u_resolution.y);
    
    vec2 vec= vec2(sin(u_time)>0.0?sin(u_time)*50.0:0.0,sin(u_time)<0.0?sin(u_time)*50.0:0.0);
    Camera c;
    c.pos = vec3(vec, 10.0*sin(u_time));   // カメラの位置
    c.dir = vec3(0.0,  0.0, -1.0);  // カメラの向き(視線)
    c.up  = vec3(sin(u_time/3.0),  cos(u_time/1.0),  0.0);// カメラの上方向
    c.side = cross(c.dir, c.up);    // 外積を使って横方向を算出
    c.depth = 1.0;          // フォーカスする深度
    c.fov = 120.0 * 0.5 * PI / 180.0;
